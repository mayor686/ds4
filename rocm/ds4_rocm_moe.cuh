...
/* gfx906 can keep both halves of a wave64 busy while avoiding two live dot
 * accumulators per lane: the low eight lanes of each 16-lane group compute
 * gate and the high eight compute up.  Each half preserves the exact block
 * assignment and quarter-wave reduction order of the regular decode kernel. */
__global__ static void moe_gate_up_mid_decode_lut_split16_kernel(
        float *gate_out,
        float *up_out,
        float *mid_out,
        const char *gate_base,
    ...
    const uint32_t lane8 = lane16 & 7u;
    const uint32_t row_lane = threadIdx.x >> 4u;
    const uint32_t row_groups = blockDim.x >> 4u;
    ...
    const uint32_t row = blockIdx.x * row_groups + row_lane;
    if (expert_i < 0) {
        if (lane16 == 0u && row < expert_mid_dim) {
            const uint64_t off = (uint64_t)pair * expert_mid_dim + row;
            if (write_aux) {
                gate_out[off] = 0.0f;
                up_out[off] = 0.0f;
    ...
    const char *base = lane16 < 8u ? gate_base : up_base;
    const cuda_block_iq2_xxs *wr =
        (const cuda_block_iq2_xxs *)(base +
            (uint64_t)expert * gate_expert_bytes +
            (uint64_t)row * gate_row_bytes);
    float value = 0.0f;
    for (uint32_t b = lane8; b < xq_blocks; b += 8u) {
        value += dev_dot_iq2_xxs_q8_K_block_lut(
            wr + b, xqb + b, s_iq2_grid, s_iq2_signs);
    }
    value = quarter_warp_sum_f32(value, lane8);

    const MASK_T mask16 = ds4_subgroup_mask(threadIdx.x, 16u);
    const float gate = __shfl_sync(mask16, value, 0, 16);
    const float up = __shfl_sync(mask16, value, 8, 16);
    if (lane16 == 0u) {
        float gate_clamped = gate;
        float up_clamped = up;
        if (clamp > 1.0e-6f) {
            if (gate_clamped > clamp) gate_clamped = clamp;
            if (up_clamped > clamp) up_clamped = clamp;
            if (up_clamped < -clamp) up_clamped = -clamp;
        }
        const uint64_t off = (uint64_t)pair * expert_mid_dim + row;
        if (write_aux) {
            gate_out[off] = gate_clamped;
            up_out[off] = up_clamped;
        }
        mid_out[off] =
            (gate_clamped / (1.0f + expf(-gate_clamped))) * up_clamped *
